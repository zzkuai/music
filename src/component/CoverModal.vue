<template>
  <transition name="fade">
    <div
      class="cover-modal-wrapper p-pf"
      v-show="visible"
      @click="hide"
    ...
    >
      ...
      <div class="cover-modal p-pr">
        ...
        <div class="modal-content f-df f-fdc">
          <ul
            class="modal-content-tag f-df f-aic"
          ...
          >
            ...
            <li
              class="modal-content-tag-item f-df f-aic"
...
export default {
  ...
  mounted() {
    this.$nextTick(async () => {
      this.color = await getImageColor({
  ...
  computed: {
    ...
    coverBlurTopStyle() {
      return {
        background: `linear-gradient(${this.color},${this.color.replace(
          /\d\)$/,
          '0.1)'
        )})`,
      };
    ...
    coverBlurBottomStyle() {
      return {
        background: `linear-gradient(${this.color.replace(/\d\)$/, '0.1)')},${
          this.color
        })`,
      };
    },
    coverFormat() {
      return `${this.cover}?param=${COVER_SIZE * PAGE_DPR}y${
        COVER_SIZE * PAGE_DPR
      }`;
    },
  },
