  ...
  <div class="cover-page">
        ...
        <label class="cover-search f-df f-aic" for="search" id="search">
          ...
          <transition name="fade">
            <span
              v-show="searchKey"
              @click="clearSearch"
              class="cover-search-close f-df f-jac"
            >
              <svg-icon
                icon-class="close"
                color="white"
                class="cover-search-close-icon size-md"
              />
            </span>
          </transition>
        </label>
    ...
    >
      <div class="content-main">
        <div
          class="content-modal transition ignore p-pa"
          :style="`backgroundColor:${modalBgColor}`"
        ></div>
        <div
          class="content-card ignore p-ph20 p-pr f-df f-jac"
          :style="`opacity:${coverOpacity}`"
        >
          <div class="content-card-left p-pr f-df f-jac" :class="{ cd }">
            <img
              v-lazy="formatCover"
              class="content-card-left-img"
              alt="cover"
            />
            <base-font
              class="content-card-left-count p-pa"
              v-if="count"
              :text="count | numberFormat"
              size="10"
              color="white"
            />
          </div>
          <div class="content-card-right f-df f-fdc f-jcb f-fx1">
            <p class="f-df f-fdc">
              <base-font
                class="content-card-right-name line-two"
                :text="name"
                color="white"
              />
              <span
                class="content-card-right-author f-df f-aic"
                @click="() => authorFunc && authorFunc()"
              >
                <img
                  v-if="avatar"
                  v-lazy="formatAvatar"
                  class="content-card-right-author-avatar"
                  alt="avatar"
                />
                <base-font
                  v-if="author"
                ...
                <svg-icon
                  ...
                  class="size-si content-card-right-icon"
                  color="white"
                ></svg-icon>
              </span>
            </p>
            <p class="f-df f-fdc">
              ...
              <span v-if="desc" class="f-df f-aic" @click="$emit('desc-click')">
                <base-font
                  v-if="desc"
                  class="content-card-right-desc line-two"
                  :text="desc"
                  color="white"
                  size="10"
                />
                <svg-icon
                  v-if="desc"
                  icon-class="arrow-forward"
                  class="size-si content-card-right-desc-icon"
                  color="white"
                ></svg-icon>
              </span>
            </p>
          ...
          <ul class="content-card-func p-pa f-df f-aic">
            <li
              class="content-card-func-item f-df f-fdc f-jac"
            ...
            >
              ...
              ></svg-icon>
              <base-font
                class="content-card-func-item-text m-mt5"
                :text="data.text"
                color="white"
              />
            </li>
        ...
        <div class="content-list-wrapper ignore p-pr">
          <ul class="content-list">
            <li
              class="content-list-item p-pv15 f-df f-aic"
            ...
            >
              ...
              <div class="content-list-item-right f-df f-fdc f-fx1">
                <base-font class="line-one" :text="data.name">
                  <base-font
                    v-if="data.alia.length > 0"
                    :text="`(${data.alia[0]})`"
                    color="text"
                  />
                </base-font>
                <base-font
                  class="content-list-item-right-author line-one"
                  :text="getArtist(data.artist)"
                  size="12"
                  color="text"
                />
              </div>
    ...
    <base-scroll v-show="searchVisible" @handle-scroll="onScroll">
      <div class="content-search">
        ...
        <div
          class="content-search-empty p-ph20 t-tac p-pf"
          v-show="searchResult === 'empty'"
        >
          <base-font
            v-show="!!searchKey"
            :text="`未找到与&quot;${searchKey}&quot;相关的内容`"
          />
        </div>
      </div>
    </base-scroll>
...
export default {
  ...
  mounted() {
    this.$nextTick(async () => {
      this.modalBgColor = await getImageMeanColor({
        imageUrl: `${this.cover}?param=10y10`,
      });
  ...
  computed: {
    ...
    formatAvatar() {
      return `${this.avatar}?param=${COVER_AUTHOR_AVATAR_SIZE * PAGE_DPR}y${
        COVER_AUTHOR_AVATAR_SIZE * PAGE_DPR
      }`;
    },
    formatCover() {
      return `${this.cover}?param=${COVER_CARD_AVATAR_SIZE * PAGE_DPR}y${
        COVER_CARD_AVATAR_SIZE * PAGE_DPR
      }`;
    },
  },
  methods: {
    ...
    onScroll(e) {
      ...
      this.isScrollMiddle =
        sTop >= (COVER_CARD_MODAL_HEIGHT - NAV_BAR_HEIGHT) / 2;
      this.coverOpacity =
        sTop <= COVER_CARD_MODAL_HEIGHT - NAV_BAR_HEIGHT
          ? (COVER_CARD_MODAL_HEIGHT - NAV_BAR_HEIGHT - sTop) /
            (COVER_CARD_MODAL_HEIGHT - NAV_BAR_HEIGHT)
          : 0;
    ...
    getSearch() {
      ...
      this.searchList = Object.freeze(
        this.list.filter(({ name, alia }) => {
