<template>
  ...
  >
    ...
    <div
      class="controller-bar ignore p-pa"
      :style="barStyle"
      :class="{ active: touchActive }"
    ></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async onTouchStart(e) {
      ...
      const touchStartOffsetX =
        ((this.touchStartX -
          this.barSize / 2 -
          this.$refs.controller.offsetLeft) *
          this.maxOffset) /
        this.controllerWidth; // 初始点击相对拖条位置
      const result =
        touchStartOffsetX >= 0 && touchStartOffsetX <= this.maxOffset
          ? touchStartOffsetX
          : -1;
    ...
    onTouchMove(e) {
      ...
      // 拖动控制条移动距离
      const moveOffset =
        this.touchStartOffsetLeft +
        ((e.changedTouches[0].clientX - this.touchStartX) * this.maxOffset) /
