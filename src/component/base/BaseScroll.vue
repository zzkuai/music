<template>
    <div class="scroll-wrapper p-pa" ref="scrollEl" @scroll="onScroll" :style="scrollStyle">
        <div class="scroll ignore p-pa" :class="{'scroll-player':playerVisible, 'nav-bar':navBar,'theme-bg':themeBg}">
            <slot></slot>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            onScroll(e) {
                ...
                if (Math.ceil(e.target.scrollTop + e.target.clientHeight) >= e.target.scrollHeight - this.bottomThreshold) {
                    // 滚动到距离底部指定位置
