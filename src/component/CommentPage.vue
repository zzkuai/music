    ...
    <base-scroll nav-bar @handle-scroll="onScroll">
      <div class="comment-card f-df">
        ...
        <div class="card-right f-df f-fx1 f-fdc f-jcc">
          ...
          <base-font
            class="card-right-author line-one"
            :text="authorFormat"
            color="text"
            size="12"
          />
      ...
      <div class="comment-wrapper theme-bg2">
        <div class="comment p-p15 theme-bg" v-if="hot && hot.length > 0">
          ...
          <ul class="comment-list o-oh">
            <li
              class="comment-list-item m-mt15 f-df"
              v-for="(data, index) in hotFormat"
              :key="index"
            >
              <div class="comment-list-item-avatar f-df">
                <img
                  class="avatar-img"
                  v-lazy="
                    `${data.avatar}?param=${userAvatarSize}y${userAvatarSize}`
                  "
                  alt="avatar"
                />
              </div>
                ...
                <div class="content-top f-df f-fdc">
                  <div class="content-top-user f-df f-jcb f-aic">
                    ...
                    <p class="f-df f-aic">
                      ...
                      <svg-icon
                        icon-class="hand"
                        class="content-icon size-ss"
                      ></svg-icon>
                    </p>
                  </div>
                  <base-font
                    class="content-top-time"
                    :text="data.time | commentTimeFormat"
                    color="text"
                    size="10"
                  />
                </div>
        ...
        <div class="comment p-p15 theme-bg">
          ...
          <ul class="comment-list o-oh" v-if="latest && latest.length > 0">
            <li
              class="comment-list-item m-mt15 f-df"
              v-for="(data, index) in latestFormat"
              :key="index"
            >
              <div class="comment-list-item-avatar f-df">
                <img
...
export default {
  ...
  computed: {
    ...
    avatarFormat() {
      return `${this.avatar}?param=${CARD_AVATAR_SIZE * PAGE_DPR}y${
        CARD_AVATAR_SIZE * PAGE_DPR
      }`;
    },
    authorFormat() {
      return this.creator
        ? `by ${this.creator}`
        : this.author.map(({ name }) => name).join('/');
    },
    hotFormat() {
  ...
  methods: {
    ...
    onScroll(e) {
      ...
      const sHotTop =
        e.target.scrollTop + NAV_BAR_HEIGHT - this.hotTitleOffset.height;
      const sLatestTop =
        e.target.scrollTop +
        NAV_BAR_HEIGHT -
        this.latestTitleOffset.height +
        (this.hotEmpty ? 0 : FIX_TEXT_HEIGHT);
