import { getSongDetail, getSongLink, getLyric, checkSong } from '@/api/page/song';
...
const getTranslateLyricFormatTimeAndText = function (str) {
    const timeArr = str.match(/\[[\d]+:[\d]+\.[\d]+]/g).map(item => item.match(/\d+:\d+\.\d+/)[ 0 ]);
    const textArr = str.split(/\[[\d]+:[\d]+\.[\d]+]/g).filter((item, index) => index > 0);
...
const actions = {
    ...
    async setPlayInfo({ commit, state, dispatch }, song) {
        ...
        const info = {
            id: song.id, name, url, alia: alia && alia[ 0 ] || '', album,
            author: artist.map(({ id, name }) => ({ id, name })), avatar,
            color, duration: playEl.duration
        };
    ...
    async setPlayLyric({ commit, state }, songId) {
        ...
        const { lrc, tlyric, nolyric, lyricUser, transUser } = await getLyric(songId);
    ...
    async setPlayDuration({ commit, state }, songId) {
        const { playEl, playCache } = state;
        let duration = 0; // 歌曲时长
        if (!playCache[ songId ]?.duration || isNaN(playCache[ songId ].duration) || playCache[ songId ].duration < 1) {
    ...
    async setNextPlaySong({ state, getters, dispatch }, payload) {
        ...
        if (playList.length < 1) {
            ...
            if (playIndex + 1 !== songIndex) {
                const totalArr = songIndex > -1 ? playList.filter((item, index) => index !== songIndex) : playList;
                const sliceIndex = songIndex > -1 && playIndex > songIndex ? playIndex : playIndex + 1;
