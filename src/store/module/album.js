...
const mutations = {
  ...
  [SET_ALBUM_LIST](state, payload) {
    ...
    const weekAlbums = all.filter(
      ({ publishTime }) => publishTime >= weekStart && publishTime <= weekEnd
    );
    const monthAlbums = all.filter(
      ({ publishTime }) => publishTime < weekStart
    );
    let monthAlbumObj = {};
    if (monthAlbums.length) {
      ...
      monthAlbumsSet.forEach(({ publishTime }) => {
        ...
        const list = monthAlbums.filter(
          ({ publishTime }) => month === new Date(publishTime).getMonth() + 1
        );
...
const actions = {
  ...
  async setAlbumComment({ commit, state }, payload) {
    ...
    const { more, comments, hotComments, total } = await getAlbumComment(
      payload
    );
    await commit(SET_ALBUM_COMMENT, {
      id: payload,
      more,
      comments,
      hotComments,
