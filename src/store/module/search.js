...
// getters
const getters = {
    getSearchResult: (state) => (keyword, type) => state?.cache[ `${ keyword }${ type }` ]?.list
};
...
const mutations = {
    ...
    [ SET_SEARCH ](state, payload) {
        ...
        if (!state.cache[ `${ keyword }${ type }` ]) state.cache[ `${ keyword }${ type }` ] = {};
    ...
    async setSearch({ commit, state }, payload) {
        ...
        if (state?.cache[ `${ keyword }${ type }` ]) {
            ...
            if (list.length < count) {
                ...
                const total = list.concat(result[ `${ searchType.find(({ code }) => code === type).type }` ]);
            ...
            const list = result[ `${ searchType.find(({ code }) => code === type).type }` ];
            const count = result[ `${ searchType.find(({ code }) => code === type).count }` ];
