...
// getters
const getters = {
    getSingerDetailById: ({ artist }) => (id) => artist[ id ].detail,
    getSingerSongById: ({ artist }) => (id) => artist[ id ].song,
    getSingerAlbumById: ({ artist }) => (id) => artist[ id ].album,
    getSingerMVById: ({ artist }) => (id) => artist[ id ].mv,
    getSingerAvatarById: ({ avatar }) => (id) => avatar[ id ],
};
...
const actions = {
    ...
    async setSingerList({ commit, state }, payload) {
        if (!state.list[ payload.cat ]) {
            await commit(SET_SINGER_LIST, { cat: payload.cat, result: { more: true, artists: [] } });
        }
        try {
            ...
            if (state.load) await commit(SET_SINGER_LOAD, false);
            await commit(SET_SINGER_LIST, { cat: payload.cat, result: Object.freeze(result) });
        } catch (e) {
            throw e;
        }
    },
    ...
    async setSingerAlbum({ commit, state }, singerId) {
        if (!state.artist[ singerId ]?.album) {
            await commit(SET_SINGER_ALBUM, { singerId, album: { list: [], more: true } });
        }
        try {
            ...
            if (state.load) await commit(SET_SINGER_LOAD, false);
            await commit(SET_SINGER_ALBUM, { singerId, album: { list: result.albums, more: result.more } });
        } catch (e) {
            throw e;
        }
    },
    async setSingerMV({ commit, state }, singerId) {
        ...
        try {
            ...
            if (state.load) await commit(SET_SINGER_LOAD, false);
            await commit(SET_SINGER_MV, { singerId, mv: { list: result.mvs, more: result.hasMore } });
        } catch (e) {
            throw e;
        }
    },
