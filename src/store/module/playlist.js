...
const getters = {
  ...
  getPlayListSongById:
    ...
    (id) =>
      detail[id].songs,
  getPlayListCreatorById:
    ({ detail }) =>
    (id) =>
      detail[id].creator,
  getPlayListSubscriberById:
    ({ detail }) =>
    (id) =>
      detail[id].subscriber,
...
const mutations = {
  [SET_PLAY_LIST](state, payload) {
    ...
    const list = playlists.map(({ id, name, coverImgUrl, playCount }) => ({
      id,
      name,
      coverImgUrl,
      playCount,
...
const actions = {
  async setPlayList({ commit, state }, payload) {
    ...
    if (
      (state.list[category] && reload) ||
      (state.list[category] && !state.list[category].more)
    )
      return;
  ...
  async setPlayListHigh({ commit, state }, payload) {
    const { category, reload } = payload;
    if (
      (state.high[category] && reload) ||
      (state.high[category] && !state.high[category].more)
    )
      return;
    const lasttime = state.high[category]?.lasttime;
    if (lasttime) await commit(SET_PLAY_LIST_LOAD, true);
    let result = await getPlayListHigh({
      cat: category,
      before: lasttime || '',
    });
    if (lasttime) {
    ...
    await commit(
      SET_PLAY_LIST_HIGH,
      Object.freeze({ ...result, cat: category })
    );
  },
  ...
  async setPlayListComment({ commit, state }, payload) {
    ...
    const { more, comments, hotComments, total } = await getPlayListComment(
      payload
    );
    await commit(SET_PLAY_LIST_COMMENT, {
      id: payload,
      more,
      comments,
      hotComments,
  ...
  async setPlayListCategory({ commit, state, dispatch }) {
    ...
    const subArr = [
      { name: '推荐', fixed: true },
      { name: '精品', fixed: true },
    ].concat(sub.filter((item, index) => index < 4));
