  ...
  <transition name="fade">
    <div class="song-more p-pf" v-show="moreVisible">
      <transition name="slide-down">
        <div
          class="song-more-content theme ignore p-pa"
          v-if="data"
          v-show="moreVisible"
        >
          <div class="song-more-header p-p15 f-df f-aic">
            <div class="song-more-header-left">
              <img
                class="song-more-header-left-img"
                :src="avatar"
                :alt="data.name"
              />
            </div>
            <div class="song-more-header-right f-fx1 f-df f-fdc">
              <base-font
                :text="`歌曲：${data.name}${
                  data.alia.length > 0 ? `（${data.alia[0]}）` : ''
                }`"
              />
              <base-font
                class="line-one m-mt5"
                :text="author"
                color="text"
                size="13"
              />
          ...
          <ul class="song-more-list">
            <li
              class="song-more-list-item f-df"
      ...
      return [
        {
          ...
          func: async () => {
            this.playNext(
              this.data,
              this.$store.getters['player/playListCount'] === 0,
              () => {
                this.hide();
        ...
        },
        {
          ...
          func: async () => {
            ...
            if (this.data.artist?.length > 1) {
              this.$emit(
                'show-singer-dialog',
                this.authorAddAvatar(this.data.artist, this.avatarObj)
              );
            }
          },
        },
