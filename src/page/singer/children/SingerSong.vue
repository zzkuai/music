<template>
    ...
    <ul class="list ignore" v-else>
        <li class="list_item f-df f-aic" v-for="(item,index) in song" :key="item.id" @click="playSong(item,song)">
            <div class="list_item-left f-df f-jac">
                <svg-icon v-if="curSongActive(item.id)" icon-class="sound" class="size-md"/>
            ...
            <div class="list_item-right f-df f-aic f-fx1">
                <div class="right-text f-df f-fdc f-jcb f-fx1">
                    ...
                    <base-font class="right-info line-one" :text="item | infoFormat" color="text" size="10"/>
                </div>
