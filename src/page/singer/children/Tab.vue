<template>
  ...
  >
    ...
    <div
      class="k-tab-content-wrapper"
      @touchstart.passive="touchStart"
      @touchmove.passive="touchMove"
      @touchend.passive="touchEnd"
    >
      <div class="k-tab-content" :style="tabContentStyle">
        <slot></slot>
      </div>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    Array.from(this.$el.querySelectorAll('.k-tab-content-item')).forEach(
      function (dom, index) {
  ...
  computed: {
    ...
    tabLineStyle() {
      const tabLineOffset =
        (this.tabContentChangeEnable &&
          (this.moveOffset * this.tabWidth) / PAGE_WIDTH) ||
        0;
      return {
        transform: `translate(${
          this.tabWidth / 2 -
          TAB_LINE_WIDTH / 2 +
          this.tabWidth * this.activeTab -
          tabLineOffset
        }px,0) translateZ(0)`,
      };
    },
    tabContentStyle() {
      const tabContentOffset =
        ((this.contentBounceEnable || this.tabContentChangeEnable) &&
          this.moveOffset) ||
        0;
      return {
        height: PAGE_HEIGHT - TAB_HEIGHT - this.offsetTop + 'px',
        transform: `translate(${
          -PAGE_WIDTH * this.activeTab + tabContentOffset
        }px,0) translateZ(0)`,
      };
    },
    tabContentChangeEnable() {
      return (
        !this.isScrolling &&
        (this.activeTab > 0 || (this.activeTab === 0 && this.moveOffset < 0)) &&
        (this.activeTab < this.tabsLen - 1 ||
          (this.activeTab === this.tabsLen - 1 && this.moveOffset > 0))
      );
    },
    tabContainerStyle() {
      return (
  ...
  methods: {
    ...
    touchMove(e) {
      ...
      this.isScrolling =
        Math.abs(touch.clientY - this.startY) >=
        CONTENT_MINIMUM_SCROLL_OFFSET_Y;
    },
    touchEnd(e) {
      ...
      this.moveOffset = touch.clientX - this.startX;
      if (
        (new Date() - this.startTime <= CONTENT_CHANGE_MINIMUM_TIME &&
          Math.abs(this.moveOffset) >= CONTENT_CHANGE_MINIMUM_OFFSET_X) ||
    ...
    changeContent() {
      const nextIndex =
        this.moveOffset > 0 ? this.activeTab - 1 : this.activeTab + 1;
