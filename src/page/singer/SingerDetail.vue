<template>
    <div class="singer-detail p-pr o-oh" ref="singer">
        <base-nav-bar :text="!singerLoad&&detail?detail.name:''" color="white" arrow-color="white"/>
        <div ref="banner"
        ...
        >
            <tab-item>
                <singer-song :class="{'player-padding':playerVisible}" v-if="!songLoad" @click-more="showSongMore"
                             :song-loading="singerLoad"></singer-song>
            </tab-item>
            <tab-item :scroll-event-enable="singerAlbumMoreEnable" @bottom-reached="loadMore('singerAlbum','getSingerAlbumById')">
                <singer-album :class="{'player-padding':playerVisible}" ref="singerAlbum" v-if="!albumLoad"></singer-album>
            </tab-item>
            <tab-item :scroll-event-enable="singerVideoMoreEnable" @bottom-reached="loadMore('singerVideo','getSingerMVById')">
                <singer-video :class="{'player-padding':playerVisible}" ref="singerVideo" v-if="!videoLoad"></singer-video>
            </tab-item>
            <tab-item>
                <singer-profile :class="{'player-padding':playerVisible}" v-if="!profileLoad" :name="detail?detail.name:''"></singer-profile>
            </tab-item>
        </tab>
        <song-more ref="more" @show-singer-dialog="showSingerDialog"></song-more>
        <base-avatar :avatar="picUrl" ref="avatar"></base-avatar>
        <base-dialog ref="dialog" type="list" title="请选择要查看的歌手" @item-on-click="goToSingerPage"></base-dialog>
    </div>
</template>
        ...
        methods: {
            async getData() {
                try {
                    ...
                    const picUrl = `${this.detail.picUrl}?param=${PAGE_WIDTH * PAGE_DPR}y${AVATAR_HEIGHT * PAGE_DPR}`;
...
<style lang="scss" scoped>
    ...
    .singer-detail-banner {
        width: 100%;
        z-index: 100;
        background: url("../../../src/asset/images/placeholder.jpg") no-repeat center center;
