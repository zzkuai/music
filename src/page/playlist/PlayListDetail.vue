        ...
        >
            <div class="subscriber p-p15 f-df f-aic f-jcb">
                <ul class="subscriber-list f-df">
                    <li class="subscriber-list-item" v-for="(data,index) in filterSubscriber" :key="index">
                        <img class="subscriber-list-item-img" :src="data.avatarUrl | filterAvatar" alt="avatar">
                    </li>
                </ul>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            items() {
                return [
                    { icon: 'play', text: '播放', func: () => this.playSong(this.song[ 0 ], this.song, true) },
                    { icon: 'comment', text: this.detail?.commentCount, func: () => this.goToComment() },
        ...
        filters: {
            filterAvatar(url) {
                return `${ url }?param=${ SUBSCRIBER_AVATAR_SIZE * PAGE_DPR }y${ SUBSCRIBER_AVATAR_SIZE * PAGE_DPR }`;
            }
        },
        methods: {
