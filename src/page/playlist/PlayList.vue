            ...
            >
                <transition name="fade">
                    <div class="p-p15" v-show="cat==='全部'">
                        <swipe-card :cards="cardList" :card-width="cardWidthFormat" @card-change="changeModalColor"/>
                    </div>
                </transition>
                <div class="playlist-high ignore f-df f-aic f-jcb" v-show="cat==='精品'">
                    <base-font class="playlist-high-title" :text="highCat" size="16"/>
                    <div class="playlist-high-filter theme-bg2 f-df f-aic" @click="showHighTag">
                        <svg-icon icon-class="filter" class="size-si"></svg-icon>
                        <base-font class="playlist-high-filter-text" text="筛选" size="12"/>
                    </div>
                </div>
                <base-loading :theme-bg="false" v-if="highLoad"></base-loading>
                <play-list-item v-else :crown="cat==='精品'" :list="cat==='全部'?itemList:list"></play-list-item>
                <base-load-more :visible="load" :theme-bg="false"></base-load-more>
            </base-scroll>
        </base-state>
        <div class="playlist-modal p-pf" :style="modalStyle"></div>
        <play-list-high-tag ref="highTag" :selected="highCat" @tag-click="changeHighCat"></play-list-high-tag>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        activated() {
            if (this.cat !== '全部' && !this.userCategory.find(({ name }) => name === this.cat)) {
        ...
        methods: {
            ...
            async changeModalColor(index) {
                const color = await getImageMeanColor({ imageUrl: `${ this.list[ index ].coverImgUrl }?param=10y10` });
