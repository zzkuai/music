  ...
  <transition name="fade">
    <div class="playlist-high-tag p-pf" v-show="visible" @click.self="hide">
      <transition name="slide-down">
        <div
          class="tag-content ignore p-pa theme-bg"
          :class="{ player: $store.state.playerVisible }"
          v-show="visible"
        >
          ...
          <ul class="tag-content-list f-df f-fww">
            <li
              class="content-list-item ignore f-df f-jac theme-bg2"
              v-for="(cat, index) in cats"
              :key="index"
              :style="index | contentListItemStyle"
              :class="{
                mg0: index % columnCount === 0,
                'theme-active': cat.name.includes(selected),
              }"
            ...
            >
              <base-font
                ...
                size="12"
                :color="
                  $store.state.theme === 'night' && !cat.name.includes(selected)
                    ? 'title'
                    : ''
                "
              />
...
const CONTENT_LIST_ITEM_WIDTH =
  (PAGE_WIDTH - 15 * 2 - CONTENT_LIST_ITEM_MARGIN * 3) / COLUMN_COUNT;
...
export default {
  ...
  filters: {
    contentListItemStyle(index) {
      const w =
        index === 0 ? CONTENT_LIST_FIRST_ITEM_WIDTH : CONTENT_LIST_ITEM_WIDTH;
      return {
        width: w + 'px',
