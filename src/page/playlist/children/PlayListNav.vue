    ...
    <nav class="playlist-nav ignore p-pf" :style="navStyle">
        <transition name="fade">
            <ul class="nav-list p-pr" v-show="visible" ref="navList">
                ...
                >
                    <base-font :text="tab.name" :color="activeTab===index?'title':'text'"/>
                </li>
                <li class="nav-line ignore p-pa theme-bg-reverse" :style="navLineStyle"></li>
            </ul>
        </transition>
    </nav>
</template>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            navLineStyle() {
                return {
                    transform: `translate3d(${ NAV_ITEM_WIDTH / 2 - NAV_LINE_WIDTH / 2 + NAV_ITEM_WIDTH * this.activeTab }px,0,0)`
                };
            },
        },
        methods: {
            ...
            changeNav(index, tab) {
                ...
                this.$nextTick(() => {
                    ...
                    this.$refs.navList.scrollTo({ left: moveLeft, top: 0, behavior: 'smooth' });
