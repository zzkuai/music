<template>
    <base-scroll :offset-top="scrollTopOffset" @bottom-reached="loadMore(false)" :scroll-event-enable="scrollEventEnable">
        <keep-alive>
    ...
    export default {
        ...
        methods: {
            async loadMore(reload) {
                ...
                try {
                    await this.setSearch({ keyword: this.keyword, type: this.type, reload });
                    this.list = Object.freeze(this.getSearchResult(this.keyword, this.type));
                } catch (e) {
                    this.reload = true;
                } finally {
