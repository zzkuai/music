<template>
    <div class="search ignore">
        <search-input @input-search="search" @cancel-search="cancelSearch" :hot-visible="hotVisible"></search-input>
        <search-nav @nav-search="search" v-if="!hotVisible"></search-nav>
        <base-state :state="state">
            <search-history @history-search="search" v-if="hotVisible"></search-history>
        ...
        methods: {
            ...
            async search(keyword, type = this.type) {
                ...
                try {
                    ...
                    if (!this.hotVisible) {
                        const list = Object.freeze(this.getSearchResult(this.keyword, this.type));
