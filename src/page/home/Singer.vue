<template>
    <base-state :state="state" :error="error" :home="true" @reload="getData(true)">
        <template v-slot:head>
            <div class="top theme p-pf ignore">
                <div class="tab-wrapper p-ph15 theme ignore">
                    <div class="tab f-df f-jcb f-aic" @click="filterVisible = !filterVisible">
                ...
                <transition name="slide-up">
                    <ul class="cat theme f-df" v-show="filterVisible">
                        <li v-for="cat in cats"
                            ...
                            @click="changeCat(cat)"
                        >
                            <base-font :text="cat.name" :color="cat.name | highLight(tabName)" size="12"/>
                        </li>
                    </ul>
                </transition>
            </div>
        </template>
        <base-scroll :scroll-event-enable="scrollEventEnable" :offset-top="scrollTopOffset" @bottom-reached="loadMore(false)">
            <com-singer class="singer-list" :list="singer" @com-singer-click="hideFilter"/>
