<template>
    <div class="play-lyric t-tac" @touchstart="lyricTouchStart" @touchend="lyricTouchEnd">
        <base-font v-if="load" text="加载中..." size="13" color="white"/>
        <template v-else-if="lyric&&lyric.length>0">
            <base-font v-for="(item,index) in lyric"
                       size="13"
                       :key="index"
                       :text="item.text"
                       color="white"
...
<script>
    ...
    export default {
        ...
        watch: {
            ...
            playCurrentTime(val) {
                if (this.lyric && !this.playHandle && this.$route.path === '/play/detail' && this.playStatus) {
                    let max = Math.max(...this.lyric.filter(({ time }) => this.isWithinRound(time, val)).map((item, index) => index), 0);
