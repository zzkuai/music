    ...
    <div class="play-control p-pa">
        <div class="play-control-time p-ph20 p-pa f-df f-aic f-jcb">
            <div class="play-control-time-start f-df f-jac">
                <base-font :text="playCurrentTime | playTimeFormat" color="white" size="10"/>
            </div>
            <controller
            ...
            <div class="play-control-time-end f-df f-jac">
                <base-font :text="playDuration | playTimeFormat" color="white" size="10"/>
            </div>
        </div>
        <ul class="play-control-btn p-pa f-df">
            <li class="play-control-btn_item f-fx1 f-df f-jac" v-for="(data,index) in playFunc" :key="index">
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            playFunc() {
                return [
                    { icon: this.playTypeIcon, size: this.playTypeIconSizeBig, func: () => this.changePlayType(true) },
                    { icon: 'previous', size: 'size-mg', func: () => this.changeSong(false) },
                    { icon: this.playIcon, size: 'size-xl', func: () => this.changePlayStatus() },
