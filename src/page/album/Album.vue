    ...
    <div class="album p-pr">
        ...
        <base-state :state="state">
            <base-scroll nav-bar :scroll-event-enable="enable" @bottom-reached="loadMore">
                <ul>
                    <li class="list-block ignore" v-for="(album,index) in albums" :key="index">
                        <base-font v-if="album.list.length && album.date" class="list-block-title m-mt15" :text="`${album.date.month}月`">
                            <base-font class="list-block-text" :text="` /${album.date.year}`" color="text" size="12"></base-font>
                        </base-font>
                        <base-font v-else class="list-block-title m-mt15" :text="album.title" size="16"></base-font>
                        <ul class="f-df f-jcb f-fww">
                            <li class="list-item p-pr f-df f-fdc"
                            ...
                            >
                                <div class="list-item-card">
                                    <img class="list-item-img"
                                         v-lazy="`${data.picUrl}?param=${listItemImgSize}y${listItemImgSize}`"
                                         alt="cover"
                                    >
                                    <base-font class="m-mt5 line-two" :text="data.name"/>
                                    <base-font class="m-mt5 line-two" :text="data.artists | artistsFilter" size="12"
                                               color="text"/>
                                </div>
                                <div class="list-item-cd ignore p-pa"
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            listItemCdStyle() {
                const endColor = this.$store.state.theme === 'day' ? 'rgba(0,0,0,0.7)' : 'rgba(255,255,255,0.04)';
                return {
