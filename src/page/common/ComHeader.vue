<template>
  <transition name="slide-up">
    <header
      class="com_header ignore theme p-pf f-df f-jcb f-aic"
      v-show="visible"
    >
      <div
        class="com_header-left com_header_item f-df f-jac"
        @click="toggleTheme"
      >
      ...
      <ul class="f-df f-fx1 f-jac">
        <router-link
          ...
          :key="nav.id"
          :to="nav.path"
          class="com_header_nav f-df f-jac"
        >
          <base-font
            :text="nav.text"
            :color="navColor(nav.path)"
            :size="navSize(nav.path)"
          />
        </router-link>
      </ul>
      <div
        class="com_header-right com_header_item f-df f-jac"
