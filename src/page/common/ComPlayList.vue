<template>
    <transition name="fade">
        <div class="com_playlist p-pf" v-show="playlistVisible">
            <transition name="slide-down">
                <div class="com_playlist-wrapper p-pa ignore" v-show="playlistVisible">
                    <div class="com_playlist-header theme f-df f-aic f-jcb ignore">
                        <div class="com_playlist-header-left f-df f-aic" @click="changePlayType(false)">
                            <div class="com_playlist-header-left-icon f-df f-aic size-mg">
                                <svg-icon :icon-class="playTypeIcon" :class="playTypeIconSizeSmall"/>
                            </div>
                            <base-font class="com_playlist-header-left-text f-df f-aic" :text="`${playTypeText}(${playListCount})`"/>
                        </div>
                        <div class="com_playlist-header-right f-df f-aic f-jce" @click="showDialog()">
                    ...
                    <div class="com_playlist-list-wrapper theme ignore" ref="list">
                        <ul class="com_playlist-list ignore">
                            <li class="com_playlist-list-item f-df f-aic f-jcb"
                            ...
                            >
                                <div class="list-item-left f-df f-aic">
                                    <svg-icon v-if="curSongActive(data.id)" icon-class="sound" class="size-md list-item-left-icon"/>
            ...
            <base-dialog ref="dialog" title="确定要清空播放列表？" confirm="清空" @confirm="cleanSong"/>
        </div>
    </transition>
</template>
        ...
        methods: {
            ...
            scrollToCurrentSong() {
                if (this.playListCount > 0) {
                    const activeElOffsetTop = document.querySelector('.list-item-active')?.offsetTop;
            ...
            async removeSong(songId) {
                const playSongIndex = this.playList.findIndex(({ id }) => id === this.playId);
